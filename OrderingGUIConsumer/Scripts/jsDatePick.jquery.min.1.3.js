...
{
    if (document.all)
    { this.isie = true; this.iever = JsDatePick.getInternetExplorerVersion() }
    else
    { this.isie = false }
    this.oConfiguration = {};
    ...
    this.setConfiguration(a); this.makeCalendar()
};
...
{
    if (this.isie && this.iever > 7) { $(a).attr("class", b) } else { a.className = b }
};
...
{
    if (typeof (a) == "function") { this.addonSelectedDelegate = a } return false
};
JsDatePick.prototype.setOnSelectedDelegate = function (a)
{
    if (typeof (a) == "function")
    {
        this.onSelectedDelegate = a; return true
    } return false
};
...
JsDatePick.prototype.setConfiguration = function (a)
{
    ...
    if (!this.oConfiguration.selectedDate)
    {
        ...
        this.currentMonth = this.oCurrentDay.month; this.currentDay = this.oCurrentDay.day
    }
...
JsDatePick.prototype.populateFieldWithSelectedDate = function ()
{
    ...
    this.lastPickedDateObject.month = this.selectedDayObject.month; this.lastPickedDateObject.year = this.selectedDayObject.year; this.closeCalendar()
};
JsDatePick.prototype.makeCalendar = function ()
{
    ...
    o = j.createElement("div"); i = j.createElement("div");
    m = j.createElement("div"); this.setC(o, "leftTopCorner");
    this.setC(i, "leftBottomCorner"); this.setC(m, "leftWall");
    this.leftWallStrechedElement = m; this.leftWall = k; this.rightWall = g;
    k.appendChild(o); k.appendChild(m); k.appendChild(i); o = j.createElement("div");
    i = j.createElement("div"); m = j.createElement("div"); this.setC(o, "rightTopCorner");
    this.setC(i, "rightBottomCorner"); this.setC(m, "rightWall");
    this.rightWallStrechedElement = m; g.appendChild(o); g.appendChild(m);
    g.appendChild(i); if (this.oConfiguration.isStripped)
    {
    ...
    e.appendChild(this.getDOMCalendarStripped()); e.appendChild(g); e.appendChild(a);
    ...
    else
    {
        ...
        else
        {
            if (jQuery.trim(b.val()).length > 5)
            {
                ...
                g = jQuery.trim(b.val()).split(c); l = e.trim().split(c);
                d = a = h = k = 0; for (d = 0; d < l.length; d++)
                {
                    switch (l[d]) { case "%d": case "%j": a = d; break; case "%m": case "%n": k = d; break; case "%M": k = d; f = true; break; case "%F": k = d; j = true; break; case "%Y": case "%y": h = d }
                }
                if (f)
                {
                    for (d = 0; d < 12; d++)
